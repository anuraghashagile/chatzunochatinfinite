...
export default function App() {
  ...
  useEffect(() => {
    const channel = supabase.channel('global-lobby-v1-counter');
    channel.on('presence', { event: 'sync' }, () => {
      const state = channel.presenceState();
      const uniqueUsers = new Set();
      Object.values(state).flat().forEach((u: any) => {
        if (u.peerId) uniqueUsers.add(u.peerId);
      });
      // Show REAL count. No fake numbers.
  ...
  }, [messages, globalMessages, activeTab, partnerTyping]);
  ...
  // --- 2. CONNECTING SCREEN (PERSONAL ONLY) ---
  if ((status === ChatMode.SEARCHING || status === ChatMode.WAITING) && activeTab === 'personal') {
    return (
      <div className="h-[100dvh] bg-white dark:bg-slate-950 flex flex-col transition-colors overflow-hidden">
        <Header 
           onlineCount={onlineCount} mode={status} theme={theme} toggleTheme={toggleTheme} onDisconnect={() => {}} partnerProfile={null} onOpenSettings={() => setShowSettingsModal(true)} onEditProfile={() => setShowEditProfileModal(true)}
  ...
  return (
    <div className={clsx(
      ...
      settings.vanishMode && activeTab === 'personal' && "dark:bg-slate-950" 
    )}>
      
      {settings.vanishMode && activeTab === 'personal' && (
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-2 w-full max-w-4xl mx-auto z-10 relative scroll-smooth">
        ...
        {activeTab === 'personal' ? (
          ...
          <>
             {/* Global Chat Content */}
             <div className="text-center text-xs text-slate-400 my-4 uppercase tracking-widest font-bold">Global Lobby</div>
             {globalMessages.map((msg) => (
               <div key={msg.id} className={clsx("flex flex-col mb-3 animate-in slide-in-from-bottom-2", msg.sender === 'me' ? "items-end" : "items-start")}>
                  <div className="text-[10px] text-slate-400 mb-0.5 px-2 font-medium">
      ...
      <div className={clsx(
        ...
        settings.vanishMode && activeTab === 'personal' ? "bg-[#1a0b2e] dark:bg-[#1a0b2e] border-purple-500/30" : "bg-white dark:bg-slate-900 border-slate-200 dark:border-white/5",
        (activeTab === 'personal' && status !== ChatMode.CONNECTED) && "opacity-50 pointer-events-none grayscale"
      )}>
        <div className="max-w-4xl mx-auto p-2 sm:p-4">
           {activeTab === 'personal' && partnerTyping && (
           ...
           <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
            ...
            <div className={clsx("relative flex-1 rounded-2xl flex items-center min-h-[50px] bg-slate-100 dark:bg-slate-800")}>
               <input
                 ...
                 placeholder={activeTab === 'personal' ? (status === ChatMode.CONNECTED ? "Type a message..." : "Disconnected") : "Message Global Chat..."}
