...
import { Infinity, Moon, Sun, Settings, ArrowLeft, Edit2, Globe, User } from 'lucide-react';
  ...
  return (
    <header className="h-16 border-b border-slate-200 dark:border-white/5 bg-white/90 dark:bg-[#05050A]/90 backdrop-blur-md flex items-center justify-between px-4 sm:px-6 sticky top-0 z-50 transition-colors font-sans shrink-0">
      ...
      <div className="flex items-center gap-3 overflow-hidden">
        {isConnected && activeTab === 'personal' && (
          <button 
        ...
        {isConnected && activeTab === 'personal' && partnerProfile ? (
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        {/* Tab Switcher (Only visible when logged in) */}
        {partnerProfile === null && mode !== ChatMode.IDLE && (
           // If connecting/searching, don't show tabs yet
           null
        )}
        ...
        {/* Logged In Actions */}
        {(mode !== ChatMode.IDLE || activeTab === 'global') && (
          <>
             ...
             <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-full mr-2">
                <button 
                  ...
                  className={clsx(
                    ...
                    activeTab === 'global' ? "bg-white dark:bg-slate-600 text-brand-500 shadow-sm" : "text-slate-400"
                  )}
                  title="Global Chat"
                >
                   <Globe size={18} />
                ...
                <button 
                  ...
                  className={clsx(
                    "p-2 rounded-full transition-all",
                    activeTab === 'personal' ? "bg-white dark:bg-slate-600 text-brand-500 shadow-sm" : "text-slate-400"
                  )}
                  title="Private Chat"
                >
                   <User size={18} />
             ...
             {activeTab === 'personal' && isConnected && (
               <button 
                onClick={onDisconnect}
